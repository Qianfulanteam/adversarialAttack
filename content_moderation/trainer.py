...
from content_moderation.shared.utils import logger
...
class Trainer:
    ...
    def __init__(
        ...
        if train_dataset:
            assert isinstance(
                train_dataset, content_moderation.datasets_.Dataset
            ), f"`train_dataset` must be of type `textattack.datasets.Dataset`, but got type `{type(train_dataset)}`."

        if eval_dataset:
            assert isinstance(
                eval_dataset, content_moderation.datasets_.Dataset
    ...
    def _generate_adversarial_examples(self, epoch):
        ...
        adversarial_dataset = content_moderation.datasets_.Dataset(
            adversarial_examples,
    ...
    def training_step(self, model, tokenizer, batch):
        ...
        _targets = targets
        targets = targets.to(content_moderation.shared.utils.device)
        ...
        if isinstance(model, transformers.PreTrainedModel) or (
            ...
            input_ids.to(content_moderation.shared.utils.device)
            logits = model(**input_ids)[0]
        else:
            input_ids = tokenizer(input_texts)
            if not isinstance(input_ids, torch.Tensor):
                input_ids = torch.tensor(input_ids)
            input_ids = input_ids.to(content_moderation.shared.utils.device)
        ...
        sample_weights = torch.ones(
            is_adv_sample.size(), device=content_moderation.shared.utils.device
        )
    ...
    def train(self):
        ...
        content_moderation.shared.utils.set_seed(self.training_args.random_seed)
        ...
        model.to(content_moderation.shared.utils.device)
    ...
    def _write_readme(self, best_eval_score, best_eval_score_epoch, train_batch_size):
        ...
        if isinstance(
            self.train_dataset, content_moderation.datasets_.HuggingFaceDataset
            ...
            self.eval_dataset, content_moderation.datasets_.HuggingFaceDataset
