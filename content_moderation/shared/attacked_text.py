...
class AttackedText:
    ...
    def pos_of_word_index(self, desired_word_idx: int) -> str:
        ...
        if not self._pos_tags:
            sentence = Sentence(
                self.text,
                use_tokenizer=content_moderation.shared.utils.TextAttackFlairTokenizer(),
            )
            content_moderation.shared.utils.flair_tag(sentence)
            self._pos_tags = sentence
        flair_word_list, flair_pos_list = content_moderation.shared.utils.zip_flair_result(
            self._pos_tags
        )
    ...
    def ner_of_word_index(self, desired_word_idx: int, model_name="ner") -> str:
        ...
        if not self._ner_tags:
            ...
            content_moderation.shared.utils.flair_tag(sentence, model_name)
            self._ner_tags = sentence
        flair_word_list, flair_ner_list = content_moderation.shared.utils.zip_flair_result(
            self._ner_tags, "ner"
        )
    ...
    def align_with_model_tokens(
        self, model_wrapper: content_moderation.models.wrappers.ModelWrapper
    ...
    def printable_text(self, key_color="bold", key_color_method=None) -> str:
        ...
        # color the key.
            ...
            if key_color_method:
                ...
                def ck(k):
                    return content_moderation.shared.utils.color_text(
