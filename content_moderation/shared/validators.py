...
from content_moderation.goal_functions import (
    InputReduction,
    MinimizeBleu,
...
MODELS_BY_GOAL_FUNCTIONS = {
    (TargetedClassification, UntargetedClassification, InputReduction): [
        r"^content_moderation.models.helpers.lstm_for_classification.*",
        r"^content_moderation.models.helpers.word_cnn_for_classification.*",
    ...
    (
        ...
        r"^content_moderation.models.helpers.t5_for_text_to_text.*",
    ],
}
...
def validate_model_gradient_word_swap_compatibility(model):
    ...
    """
    if isinstance(model, content_moderation.models.helpers.LSTMForClassification):
        return True
    else:
...
def transformation_consists_of(transformation, transformation_classes):
    ...
    from content_moderation.transformations import CompositeTransformation
...
def transformation_consists_of_word_swaps(transformation):
    ...
    swaps."""
    from content_moderation.transformations import WordSwap, WordSwapGradientBased
...
def transformation_consists_of_word_swaps_and_deletions(transformation):
    """Determines if ``transformation`` is a word swap or consists of only word
    swaps and deletions."""
    from content_moderation.transformations import WordDeletion, WordSwap, WordSwapGradientBased
...
def transformation_consists_of_word_swaps_differential_evolution(transformation):
    from content_moderation.transformations import WordSwapDifferentialEvolution
