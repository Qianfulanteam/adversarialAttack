...
#
# Models hosted by content_moderation.
...
class ModelArgs:
    ...
    def _create_model_from_args(cls, args):
        ...
        ``content_moderation.models.wrappers.ModelWrapper`` object."""
        ...
        if args.model_from_file:
            ...
            if not isinstance(model, content_moderation.models.wrappers.ModelWrapper):
                raise TypeError(
                    f"Variable `{model_name}` must be of type "
                    f"``content_moderation.models.ModelWrapper``, got type {type(model)}."
                )
            ...
            model = content_moderation.models.wrappers.HuggingFaceModelWrapper(model, tokenizer)
            ...
            if args.model.startswith("lstm"):
                ...
                model = content_moderation.models.helpers.LSTMForClassification.from_pretrained(
                    args.model
                )
                ...
                model = (
                    content_moderation.models.helpers.WordCNNForClassification.from_pretrained(
                        args.model
                    )
                )
            elif args.model.startswith("t5"):
                model = content_moderation.models.helpers.T5ForTextToText.from_pretrained(
                    args.model
                )
            else:
            ...
            # a HuggingFace model).
            if isinstance(model, content_moderation.models.helpers.T5ForTextToText):
                model = content_moderation.models.wrappers.HuggingFaceModelWrapper(
                    model, model.tokenizer
                )
            else:
                model = content_moderation.models.wrappers.PyTorchModelWrapper(
                    model, model.tokenizer
                )
            ...
            if os.path.exists(os.path.join(args.model, "t5-wrapper-config.json")):
                ...
                if (
                    ...
                    model = eval(
                        f"content_moderation.models.helpers.{model_class}.from_pretrained({args.model})"
                    )
                    model = content_moderation.models.wrappers.PyTorchModelWrapper(
                        model, model.tokenizer
                    )
        ...
        assert isinstance(
            model, content_moderation.models.wrappers.ModelWrapper
        ), "`model` must be of type `content_moderation.models.wrappers.ModelWrapper`."
        return model
