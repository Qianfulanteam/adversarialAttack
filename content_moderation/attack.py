...
from content_moderation.attack_results import (
    FailedAttackResult,
    MaximizedAttackResult,
    SkippedAttackResult,
    SuccessfulAttackResult,
)
from content_moderation.constraints import Constraint, PreTransformationConstraint
from content_moderation.goal_function_results import GoalFunctionResultStatus
from content_moderation.goal_functions import GoalFunction
from content_moderation.models.wrappers import ModelWrapper
from content_moderation.search_methods import SearchMethod
from content_moderation.shared import AttackedText, utils
from content_moderation.transformations import CompositeTransformation, Transformation
...
class Attack:
    ...
    def __init__(
        ...
        for constraint in constraints:
            if isinstance(
                constraint,
                content_moderation.constraints.PreTransformationConstraint,
            ):
    ...
    def cuda_(self):
        ...
        def to_cuda(obj):
            visited.add(id(obj))
            if isinstance(obj, torch.nn.Module):
                obj.to(content_moderation.shared.utils.device)
            elif isinstance(
                obj,
                (
                    Attack,
