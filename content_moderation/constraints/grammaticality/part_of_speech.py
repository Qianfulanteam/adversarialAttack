...
from content_moderation.constraints import Constraint
from content_moderation.shared.utils import LazyLoader, device
from content_moderation.shared.validators import transformation_consists_of_word_swaps
    ...
    def _get_pos(self, before_ctx, word, after_ctx):
        ...
        if context_key in self._pos_tag_cache:
            ...
            if self.tagger_type == "flair":
                ...
                word_list, pos_list = content_moderation.shared.utils.zip_flair_result(
                    context_key_sentence
                )

            if self.tagger_type == "stanza":
                word_list, pos_list = content_moderation.shared.utils.zip_stanza_result(
