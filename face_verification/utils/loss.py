...
def loss_adv(loss_name, outputs, labels, target_labels, target, device):
    ...
    if loss_name=="ce":
        ...
        if target:
            '''目标攻击
            outputs：adv_embedding,
            target_labels：target_embedding'''
            cos_sim = F.cosine_similarity(outputs, target_labels)
            cost = cos_sim.mean()
        else:
            '''非目标攻击
            outputs：adv_embedding,
            labels：true_embedding'''
            cos_sim = F.cosine_similarity(outputs, labels)
            cost = -cos_sim.mean()
    return cost
