...
graph TD
    A[初始化] --> B[二分搜索确定c]
    B --> C[随机初始化w]
    C --> D[计算扰动δ=0.5*tanh w+0.5-x]
    D --> E[前向传播计算f(x+δ)]
    E --> F[计算目标函数 L=‖δ‖₂+c⋅f(x+δ)]
    F --> G[反向传播更新w]
    G --> H{满足停止条件?}
    H -- 否 --> D
    H -- 是 --> I{攻击成功?}
...
class CW(object):
    ...
    def __init__(self, model, device='cuda', norm=2, kappa=0, lr=0.2, init_const=0.01,
        ...
        '''
        δ：扰动，对应代码中的 perturbed_images - original_images。
        c：平衡参数，对应代码中的 const_origin。
        f(x+δ,y)：损失函数，对应代码中的 constraint_loss。
        κ：置信度参数，对应代码中的 kappa。
        w：通过tanh变换的优化变量，对应代码中的 var_images + modifier（因为var_images是原始图像的tanh逆，而modifier是优化变量）。
        '''
    ...
    def __call__(self, images=None, labels=None, target_labels=None):
        ...
        if targeted:
            ...
            targeteg_class_in_one_hot = torch.stack(targeteg_class_in_one_hot).clone().type_as(images).to(
                self.device)  # torch.Size([10, 10])
        else:
            for i in range(batch_size):
                current_target = temp_one_hot_matrix[copy_labels[i]]
                targets_in_one_hot.append(current_target)
            targets_in_one_hot = torch.stack(targets_in_one_hot).clone().type_as(images).to(
                self.device)  # torch.Size([10, 10])
        ...
        for search_for_c in range(self.binary_search_steps):
            ...
            for iteration_times in range(self.max_iter):
                ...
                if targeted:
                    constraint_loss = torch.max((prediction - 1e10 * targeteg_class_in_one_hot).max(1)[0] - (
                                prediction * targeteg_class_in_one_hot).sum(1),
                ...
                optimizer.step()
                # 如果当前扰动导致更小的L2距离且攻击成功，则更新最佳扰动图像。
